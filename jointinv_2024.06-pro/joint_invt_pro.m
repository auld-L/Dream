% This is the main script to conduct the rupture inversion process, which
% allows to adopt data of TELESEISMIC record, STRONG-MOTION record, GPS 
% record and INSAR record JOINTLY or SINGLY. 

% Programmed by Yong Zhang, packed by Pingchuan Wang
% 2018/11 at Peking University
% Updated by Sibo Hua,2021.03.15 at Peking University
% Updated by Sibo Hua,2021.09.06 at Peking University
%% Parameters of thefault plane and other information (1)

% Location of the epicenter, epi = [latitude,longitude] (1.1)
close all;
clear all;
epi = [31.0636,103.3718];
%epi = [31.018,103.365];
% Depth of the epicenter in km (1.2)
%depth = 7.6;
%depth=14.34;
% Parameters of the fault plane setting, fault = [strike,dip,rake], all in 
% degree (1.3)
%fault = [54,70,11];
gridsize=[10,10];
load('faultwenchuannew2.mat');%额外准备的断层信息数据
faultbian=changefaultbian(faultbian,gridpai);%平滑使每个子断层边界重合
save('gridpai.mat','gridpai');
%Sall=consids(faultbian,faultall,gridsize);%计算每个子断层的相对面积（不可用于倾角接近90°的情况）
Sall=considsplus(faultbian,faultall,gridsize);%计算每个子断层的相对面积
save('Sall.mat','Sall');

srate=1;%震源时间函数采样率
fault = [231,35,135];%%此fault中只有fault（3）对应滑动角起作用
dist=epi-faultall(:,1:2);
dL=sqrt(dist(:,1).^2+dist(:,2).^2*(cosd(epi(1)))^2);
epidex=find(dL==min(dL));
epi=faultall(epidex,1:3);%计算找到距离给定震中最近的子断层，并将其作为震源位置。

depth=epi(3);
epi(3)=[];
source=biaoall(epidex,2:3);
% Numbers of subfaults along both direction, grid = numbers of subfaults 
% [along dip,along strike] (1.4)
%grid = [7,60];
%faultallori=faultall;
% faultall(:,4)=231;
% faultall(:,5)=35;
% Size of each subfault in km, gridsize = length of each subfault [along 
% dip,along strike] (1.5)
% gridsize = [10,10];

% Which subfault the source locates, source = the ordinal number [along 
% dip,along strike] of the subfault (1.6)
%source = [3,42];

% In fact, the upper edge of the fault plane in our model is on the
% ground, so the ordinal number along dip where the source locates is
% determined by the depth and dip. DO NOT CHANGE UNLESS NECESSARY. (1.7)
%source(1) = round(depth/sind(fault(2))/gridsize(1)+1/2);

% Determine to invert in velocity (isdis = 0) or displacement (isdis = 1)
% form (1.8)
isdis = 1;

% Name of the file the inversion results to save in (1.9)
savefile = 'result_jointwenchuannew.mat';
%% Parameters of types of records (2)
% PARAMETERS OF TELESEISMIC RECORDS
% Weight of the records in the joint inversion. If the records are not
% included, then set 0.(2.1)
weight_tele = 0.5;
teletp=1;%%%远震格林函数的计算方式：每个子断层到每个台站精准计算（teletp=1）或用震源处的格林函数做路径近似（teletp=0）。
ifastf=0;%%%视震源时间函数反演,不可用
weightastf=0.1;
% Bandpass of the records in Hz (2.2)
band_tele = [0.01,0.15];

% Name of file the observed record saved in (2.3)
file_tele = 'ob_telewenchuan.mat';

% Path of the Green's function for the inversion (2.4)
%gpath_tele = 'D:\matlab2021\codes\Green_tele_ak135\green_func\';
gpath_tele='D:\matlab2021\codes\jtinew2022.09-3d\Green_tele\green_func\';
% Allow (set 1) or not allow (set 0) the observed records to shift to a 
% time when they fit the synthetic waves the best after an initial
% inversion, and then conduct a second inversion calculation (2.5)
timeshift_tele = 1;

% Calculate (set 1) the Green's function of the records or not (set 0) if
% the calculation has been done and no need to change (2.6) 
ifcalg_tele = 0;

weight_wphase=1;
%band_wphase=[0.0017,0.0067];
band_wphase=[0.0017,0.01];
file_wphase='ob_WphasewenchuanLHnews0.5.mat';
%gpath_wphase='D:\matlab2021\codes\Green_tele_ak135\green_func\';
gpath_wphase='D:\matlab2021\codes\jtinew2022.09-3d\Green_tele\green_func\';
timeshift_wphase=1;
ifcalg_wphase=0;
wphasetp=1;
% PARAMATERS OF STRONG-MOTION RECORDS
% Weight of the records in the joint inversion. If the records are not
% included, then set 0.(2.7)
weight_sm =1;

% Bandpass of the records in Hz (2.8)
band_sm = [0.02,0.10];

% Name of file the observed record saved in (2.9)
file_sm = 'ob_smwcsnew.mat';

% Path of the Green's function for the inversion (2.10)
gpath_sm = 'D:\matlab2021\codes\jtinew2022.09\Green_sm\green_func\';

% Allow (set 1) or not allow (set 0) the observed records to shift to a 
% time when they fit the synthetic waves the best after an initial
% inversion, and then conduct a second inversion calculation (2.11)
timeshift_sm = 1;

% Calculate (set 1) the Green's function of the records or not (set 0) if
% the calculation has been done and no need to change (2.12) 
ifcalg_sm = 0;

% PARAMETERS OF GPS RECORDS
% Weight of the records in the joint inversion. If the records are not
% included, then set 0.(2.13)
weight_gps = 1;

% Name of file the observed record saved in (2.14)
file_gps = 'gps.mat';

% PARAMETERS OF INSAR RECORDS
% Weight of the records in the joint inversion. If the records are not
% included, then set 0.(2.15)
weight_insar =0;

% Name of file the observed record saved in (2.16)
file_insar = 'insar.mat';

%% Parameters of the inversion calculation (3)

% Total length of source time function of the subfaults in s (3.1)
lensub = 120;

% Maximum rupture time of the subfaults in s (3.2)
lenrup = 30;

% Maximum rupture velocity in km/s (3.3)
vrup = 4;


%lambda1=120;
lambda1=60;
% Temporal-smooth factor (3.5)

%lambda2=240;
lambda2=120;

% Minimum scalar moment restriction factor (3.6)
lambda3 = 0.01;

% Allow (set 1) the rake vary in a ±45 degree range or not (set 0) (3.7)
rakevar =1;%%if use astf inversion，rakevar must be 0;

% Set slip on any edge of the fault plane to 0. 0 for setting no edge to 0,
% 1 for setting all edges except for the upper edge to 0, 2 for setting all
% edges to 0. (3.8)
set_edge0 = 1;%无用


%% Parameters of some of the figures (4)

% Range of the map in Figure 2, or just keep them empty for a default
% setup (4.1)
% lon_smsta = [171,176];
% lat_smsta = [-44,-40];
lon_smsta = [];
lat_smsta = [];

% rct = numbers of [rows,column] in Figure 5, or just keep it empty for a
% default setup (4.2)
rct = [];

% rct = numbers of [rows,column] in Figure 6, or just keep it empty for a
% default setup (4.3)
rcsm = [];

% Range of the map in Figure 7, or just keep them empty for a default
% setup (4.4)
lon_gps = [];
lat_gps = [];

% Ticks interval (in degree) of the maps in Figure 8 (4.5)
dlon_sar = 0.1;
dlat_sar = 0.1;

%% Inversion calculations (5)
[miu1,miun] = getmiud(depth,faultall,gridsize,source,epi);%复杂断层情况下的剪切模量计算
if ifastf==1
    [obt,gt,locat,mmt,s1,leng,t,t0] = getobg_tele4(weight_tele,ifcalg_tele,file_tele,...
    epi,depth,fault,grid,gridsize,source,gpath_tele,miun,band_tele,isdis);
    tdt=t-repmat(t0',size(t,1),1);
    
    
    if teletp==1
[obt,gt,locat,mmt,s1,leng] = getobg_tele2(weight_tele,ifcalg_tele,file_tele,...
     epi,depth,fault,grid,gridsize,source,gpath_tele,miun,band_tele,isdis);%%%%%
    end
elseif teletp==0
 [obt,gt,locat,mmt,s1,leng] = getobg_tele3d(weight_tele,ifcalg_tele,file_tele,...
     epi,depth,fault,faultall,gridsize,source,gpath_tele,miun,band_tele,isdis);%%%%
elseif teletp==1
    [obt,gt,locat,mmt,s1,leng] = getobg_tele2d(weight_tele,ifcalg_tele,file_tele,...
     epi,depth,fault,faultall,gridsize,source,gpath_tele,miun,band_tele,isdis);
end
if wphasetp==1%精确计算每个子断层W震相格林函数（建议使用）
    [obw,gw,locaw,mmw,sw,lengw,twstop] = getobg_wphase1d(weight_wphase,ifcalg_wphase,file_wphase,...
     epi,depth,fault,faultall,gridsize,source,gpath_wphase,miun,band_wphase,isdis);
 
elseif wphasetp==0%路径近似计算W震相格林函数（不建议使用）
    [obw,gw,locaw,mmw,sw,lengw,twstop] = getobg_wphase0d(weight_wphase,ifcalg_wphase,file_wphase,...
     epi,depth,fault,faultall,gridsize,source,gpath_wphase,miun,band_wphase,isdis);
end


[obsm,gsm,locasm,mmsm,s2] = getobg_smd(weight_sm,ifcalg_sm,file_sm,...
    epi,depth,fault,faultall,gridsize,source,gpath_sm,miun,band_sm,isdis);%
Lenori=max(size(obsm,1),size(obt,1));
[obt,gt] = length_unification(obt,gt,obw);%

if weight_sm
    [obsm,gsm] = length_unification(obsm,gsm,obw);%
    %mmsm=mmsm(:,5:8);locasm(:,3)=[];
    if weight_tele
        [obt,gt] = length_unification(obt,gt,obsm);
    end
end
[obt,gt] = length_unification(obt,gt,obw);
difsrate=[zeros(1,size(obt,2))+s1/srate,zeros(1,size(obsm,2))+s2/srate,zeros(1,size(obw,2))+sw/srate];%差异采样率记录

ob = [obt,obsm,obw];g = cat(4,gt,gsm,gw);loca = [locat;locasm;locaw];mm = [mmt;mmsm;mmw];

clear gt gsm

[ob,g,mob] = normalize_obgw2(ob,g,obt,obsm,obw,weight_tele,weight_sm,weight_wphase,difsrate);%平权和加权
if weight_wphase
    ob1=ob(1:Lenori,1:size(obt,2)+size(obsm,2));
    g1=g(1:Lenori,:,:,1:size(obt,2)+size(obsm,2));
    loca1=loca(1:size(obt,2)+size(obsm,2),:);
    [G1,sGDT0,hdata1,meangreen1,sumob1,trup_con,locasub,locadep,trup] = get_Gmatrixd(...
    ob1,g1,loca1,epi,biaoall,fault,faultall,gridsize,source,lensub,lenrup,vrup,srate,...
    lambda1,lambda2,lambda3,rakevar,joinpoint,difsrate(1:size(obt,2)+size(obsm,2)));%W震相以外的地震波数据格林函数反演矩阵构建

    ob2=ob(:,size(obt,2)+size(obsm,2)+1:end);
    g2=g(:,:,:,size(obt,2)+size(obsm,2)+1:end);
    loca2=loca(size(obt,2)+size(obsm,2)+1:end,:);
    [G2,sGDT0,hdata2,meangreen2,sumob2,trup_con,locasub,locadep,trup] = get_Gmatrixd(...
    ob2,g2,loca2,epi,biaoall,fault,faultall,gridsize,source,lensub,lenrup,vrup,srate,...
    lambda1,lambda2,lambda3,rakevar,joinpoint,difsrate(size(obt,2)+size(obsm,2)+1:end));%W震相格林函数反演矩阵构建
LL=size(ob2,1);
k=0;
dexw=[];
for i=1:size(ob2,2)
%     dex=find(ob2(:,i)==0);
%     dex1=dex+1;
%     dex0=dex(2:end)-dex1(1:end-1);
%     ddd=find(dex0~=0);
%     if isempty(ddd)
%         dexo=dex(2)+2;
%     else
%         dexo=dex(ddd(end)+2);
%     end
    dexo=twstop(i)+1;
    
    hdata2e(k+1:k+dexo)=hdata2((i-1)*LL+1:(i-1)*LL+dexo);
    G2e(k+1:k+dexo,:)=G2((i-1)*LL+1:(i-1)*LL+dexo,:);
    dexw(i)=dexo;
    k=k+dexo;%删去W震相格林函数矩阵多余的0，提升计算效率
end
save dexw.mat dexw
faultmg=faultall(:,4:5);
faultmg(:,3)=fault(3);
meangreen = get_Gmatrixmeangreen(...
    ob,g,faultmg);
hdata1=hdata1*meangreen1/meangreen;
%hdata2=hdata2*meangreen2/meangreen;
hdata2e=hdata2e*meangreen2/meangreen;
obc=[hdata1;hdata2e'];
G1=G1*meangreen1/meangreen;
G2e=G2e*meangreen2/meangreen;
G2=G2*meangreen2/meangreen;
G=[G1;G2e];
sumob = obc(:)'*obc(:);%
else
[G,sGDT0,hdata,meangreen,sumob,trup_con,locasub,locadep,trup] = get_Gmatrixd(...
    ob,g,loca,epi,biaoall,fault,faultall,gridsize,source,lensub,lenrup,vrup,srate,...
    lambda1,lambda2,lambda3,rakevar,joinpoint);%
end
if ifastf==1
    obtele=ob(:,1:size(locat,1));
    gtele=g(:,:,:,1:size(locat,1));
    [astftele,synastf]=getastftele(obtele,gtele,source,grid,fault,lensub,band_tele,s1);
    
    
    Gastf=get_stfinvG(grid,trup_con,tdt,lensub*srate);
    astftele=astftele(:)/srate;
    mobstf=max(max(Gastf))/weightastf/max(max(G))*2;
    astftele=astftele/mobstf;
    Gastf=Gastf/mobstf;
    grida=grid;
    save('astfneed.mat','gtele','source','grida','fault','lensub','band_tele','s1','rakevar','mobstf');
else
    Gastf=[];
    astftele=[];
end
[slipg,Ggps,weit_gps,locagps] = getobg_gps2d(weight_gps,file_gps,sumob,...
    locasub,locadep,fault,faultall,gridsize,trup_con,rakevar,1);%

[slipsar,Gsar,weit_sar,locasar,nsar,dr] = getobg_insar2d(weight_insar,file_insar,...
    sumob,locasub,locadep,fault,faultall,gridsize,trup_con,rakevar);%
weit_gps=weit_gps/(sqrt(weight_tele^2+weight_sm^2+weight_wphase^2));
G_jt = [G;Ggps*weit_gps;Gsar*weit_sar];%
clear g G
sGDT=[Gastf;sGDT0];
if weight_wphase
    hdata_use = [obc(:);slipg(:)*weit_gps;slipsar*weit_sar;astftele(:);zeros(...
    size(sGDT0,1),1)];%
else
hdata_use = [ob(:)/meangreen;slipg(:)*weit_gps;slipsar*weit_sar;astftele(:);zeros(...
    size(sGDT0,1),1)];%
end
tic;

    [Xsolve,rsq,~,~,~,~,~,~]=pl_vt_spar2(G_jt,sGDT,hdata_use,1e-10,500,'X(X<0)=0;');
toc


load('DT.mat');
    rough0d=D*Xsolve;
    rough0t=T*Xsolve;
    rough1d=norm(rough0d)/norm(Xsolve);
    disp(['D',num2str(rough1d)]);
    rough1t=norm(rough0t)/norm(Xsolve);
    disp(['T',num2str(rough1t)]);%计算时空光滑粗糙度



if weight_wphase
    syn0=G_jt(1:numel(obc),:)*(Xsolve*meangreen);
    syn1=reshape(syn0(1:Lenori*(size(obt,2)+size(obsm,2))),Lenori,size(obt,2)+size(obsm,2));
    syn2=G2*(Xsolve*meangreen);
    syn=zeros(size(ob));
    syn(1:Lenori,1:size(obt,2)+size(obsm,2))=syn1;
    syn(:,size(obt,2)+size(obsm,2)+1:end)=reshape(syn2,size(obw,1),size(obw,2));%计算合成波形
else
syn = reshape(G_jt(1:numel(ob),:)*(Xsolve*meangreen),size(ob));%
end
%plot(rsq);
%saveas(gcf,'rsq1','fig');
% [Xsolve,ob0,syn,astftele2,synastf2] = shift_ob(timeshift_tele*weight_tele,timeshift_sm*...
%     weight_sm,Xsolve,ob,syn,obt,obsm,hdata_use,meangreen,G_jt,sGDT,srate,ifastf);%
[Xsolve,ob0,syn,astftele2,synastf2] = shift_obw2(timeshift_tele*weight_tele,timeshift_sm*...
    weight_sm,timeshift_wphase*weight_wphase,Xsolve,ob,syn,obt,obsm,obw,hdata_use,meangreen,G_jt,sGDT,srate,ifastf,Lenori);%时移后再次计算
load('DT.mat');
    rough0d=D*Xsolve;
    rough0t=T*Xsolve;
    rough1d=norm(rough0d)/norm(Xsolve);
    disp(['D',num2str(rough1d)]);
    rough1t=norm(rough0t)/norm(Xsolve);
    disp(['T',num2str(rough1t)]);%计算时空光滑粗糙度
if weight_wphase
    syn0=G_jt(1:numel(obc),:)*(Xsolve*meangreen);
    syn1=reshape(syn0(1:Lenori*(size(obt,2)+size(obsm,2))),Lenori,size(obt,2)+size(obsm,2));
    syn2=G2*(Xsolve*meangreen);
    syn=zeros(size(ob));
    syn(1:Lenori,1:size(obt,2)+size(obsm,2))=syn1;
    syn(:,size(obt,2)+size(obsm,2)+1:end)=reshape(syn2,size(obw,1),size(obw,2));%合成波形
else
syn = reshape(G_jt(1:numel(ob),:)*(Xsolve*meangreen),size(ob));%
end

if ifastf==1
synstf=sGDT(1:size(obt,2)*lensub*srate,:)*Xsolve*mobstf*srate;
%synastf2=synastf2;
end

misfta0=sum(sum((ob0-syn).^2))/sum(sum(ob0.^2));

[X,ob,syn] = get_obgX(Xsolve,ob0,syn,mob,lensub,trup_con,size(faultall,1),srate,rakevar);%%%%try
if weight_sm
obsm = ob(1:size(obsm,1),size(obt,2)+1:size(obt,2)+size(locasm,1));
synsm = syn(1:size(obsm,1),size(obt,2)+1:size(obt,2)+size(locasm,1));
obsm = reshape(obsm,[size(ob,1),size(obsm,2)/3,3]);
obsm = permute(obsm,[1,3,2]);
synsm = reshape(synsm,[size(ob,1),size(obsm,3),3]);
synsm = permute(synsm,[1,3,2]);
end
if weight_gps
syng = Ggps*Xsolve;
end
if weight_insar
synsar = Gsar*Xsolve;
end

%[X,ob,syn] = get_obgX(Xsolve,ob,syn,mob,lensub,trup_con,prod(grid),srate);

%[slip,stf,slipx,slipy,slipe,slipn,zslip] = get_result(X,lensub,grid,...
%    gridsize,fault,miun,srate);
[slip,stf,slipx,slipy,slipe,slipn,zslip,stfsub_out] = get_result2d(X,lensub,faultall,...
    gridsize,fault,miun,srate);
stfsub_out=stfsub_out*srate;
mw = m2m(sum(slip(:).*miun(:).*Sall(:)*prod(gridsize)*1e6));

msftj=sum(sum((ob-syn).^2))/sum(sum((ob).^2));
% if weight_tele
% obtelef=ob(:,1:size(obt,2));
% syntele=syn(:,1:size(obt,2));
% msftt=sum(sum((obtelef-syntele).^2))/sum(sum((obtelef).^2));
% msftti=sum((obtelef-syntele).^2)./sum((obtelef).^2);
% end
% if weight_sm
% msftsm=sum(sum(sum((obsm-synsm).^2)))/sum(sum(sum((obsm).^2)));
% msftsmi(:,:)=sum((obsm-synsm).^2)./sum((obsm).^2);
% end
%% Figures (6)

clear G_jt G_jte G gw g ncst sGDT k Area Gsar Ggps G1 G2 G2e g1 g2
stf=sum(stfsub_out/3e10*diag(miun(:).*Sall(:)),2);
save(savefile)
%faultall=faultallori;
%[dex,dey]=substfs_plot(epi,grid,gridsize,source,stfsub_out,srate);
%saveas(gcf,'stfsub','fig');
% Figure 2: Distribution of the teleseismic stations
if weight_tele
    figure
    epi_sta00(epi,loca(1:size(obt,2),:),mm(1:size(obt,2),:),100);
    title('Teleseismic Stations');
    %saveas(gcf,'locatele','fig');
end
if weight_wphase
    figure
    epi_sta00(epi,locaw(1:size(obw,2),:),mmw(1:size(obw,2),:),100);
    title('Wphase stations');
    %saveas(gcf,'locatele','fig');
end
% Figure 3: Distribution of the strong-motion stations
if weight_sm
    plt_sm_sta(lon_smsta,lat_smsta,epi,locasm,mmsm);
    %saveas(gcf,'locasm','fig');
end

% Figure 4: Static slip distribution
figure
[numth,interval,m] = colornumth(slip);
% numth=6;
% interval=0.5;
% [slip01,start,stop] = faultslip_pcolor(grid,gridsize,source,zslip,...
%     (numth)*interval,numth,interval);
%  [slip01,start,stop] = faultslip_pcolor3d(faultall,gridsize,source,zslip,...
%      (numth)*interval,numth,interval);
 faultslip_pcolor3d(faultall,faultbian,gridsize,epidex,zslip,10);%复杂子断层滑动分布
% axis equal tight
% set(gca,'linewidth',0.5,'tickdir','in','xminortick','off','yminortick',...
%     'off','fontsize',16);
%saveas(gcf,'rup','fig');

% Figure 5: Source time function
if ~isempty(ob)
    figure
    fill(0:1/srate:lensub,[stf;0],'c');
    set(gca,'layer','top');
    set(gca,'fontsize',15);
    title(['Source Time Function',num2str(mw,'%.2f')]);
    xlabel('Time(s)');
    ylabel('Moment rate(Nm/s)');
    %saveas(gcf,'stf','fig');
end

% Figure 6: Fitting of teleseismic waves
if weight_tele
    if isempty(rct)
        rct = [6,ceil(size(obt,2)/6)];
    end
    leng=leng*srate;
    [misfitt,cort]=pltobsyn_new(ob(1:leng,1:size(obt,2)),syn(1:leng,1:size(obt,2)),...
        (1:leng)/srate,rct,0.05,mm(1:size(obt,2),:),repmat('\itP/Z',...
        [size(obt,2),1]));
    %saveas(gcf,'syntele','fig');
end
if weight_wphase
    rcw=[];
    if isempty(rcw)
        rcw = [7,ceil(size(obw,2)/7)];
    end
    lengw=lengw*sw;
    obd=ob(1:lengw,end-size(obw,2)+1:end);
    synd=syn(1:lengw,end-size(obw,2)+1:end);
    for i=1:size(synd,2)
        synd(twstop(i)+1:end,i)=0;
    end
    [distpai,azi]=distance(epi,locaw);
    [distout,dexw]=sort(distpai,'ascend');
    [misfitw,corw]=pltobsyn_new(obd(:,dexw),synd(:,dexw),...
        (1:lengw)/srate,rcw,0.05,mmw(dexw,:),repmat('\itW/Z',...
        [size(obw,2),1]));
end
% Figure 7: Fitting of strong-motion waves
if weight_sm
    obsm(301:end,:,:)=[];
    synsm(301:end,:,:)=[];
    if isempty(rcsm)
        rcsm = [4,ceil(size(obsm,3)/4)];
    end
    [misfitsm,corsm]=pltobsyn3_new(obsm,synsm,0:1/srate:(size(obsm,1)-1)/srate,rcsm,0.05,...
        mm(size(obt,2)+1:size(obt,2)+size(obsm,3),:));
    %saveas(gcf,'synsm','fig');
end

% Figure 8: Fitting of GPS records
if weight_gps
    matching_gpsd([102,106.5],[30,34],locagps,10,epi,faultall,zslip,slipg,syng,faultbian);
    title('GPS Data Fitting');
end

% Figure 9: Fitting of InSAR records
if weight_insar
%     plot([slipsar,synsar])
    matching_insar(dlon_sar,dlat_sar,slipsar,synsar,locasar,nsar,dr);
%     title('InSAR Data Fitting');
end

disp(['The Mw calculated is ',num2str(mw,'%.2f'),'!'])

% n1=8;%时间间隔（s）
% n2=3;%图行数
% n3=5;%图列数
% stfsub_out=stfsub_out(:)/3e16/prod(gridsize);
% maxslip=max(max(slip));
% %Figure 10 每n1秒时间段内的累计滑动量
% rate=rup_pltslipshot2(stfsub_out,grid,lensub*srate,[n1*srate,n2,n3],1,maxslip/2,12,srate);
% colorbar('southoutside','position',[0.27    0.0516    0.5    0.0308]);
% %Figure 11 截止到不同个n1秒时间的累计滑动量
% rate=rup_pltslipshot2(stfsub_out,grid,lensub*srate,[n1*srate,n2,n3],2,maxslip,12,srate);
% colorbar('southoutside','position',[0.27    0.0516    0.5    0.0308]);
% %%Figure 12、13 视震源时间函数反演拟合
% if ifastf==1
%     if isempty(rct)
%         rct = [4,ceil(size(obt,2)/4)];
%     end
%     %leng=leng*srate;
%     if size(synastf2,1)>0
%         synastf=synastf2;
%         astftele=astftele2;
%     end
%     astftele=reshape(astftele,lensub*srate,size(obt,2));
%     synstf=reshape(synstf,lensub*srate,size(obt,2));
%     astftele=astftele*mobstf*srate;
%     
%     pltobsyn_new(ob0(1:leng,1:size(obt,2)),synastf(1:leng,1:size(obt,2)),...
%         (1:leng)/srate,rct,0.05,mm(1:size(obt,2),:),repmat('P/Z',...
%         [size(obt,2),1]));
%     rct = [4,ceil(size(obt,2)/4)];
%     pltobsyn_new(astftele(1:lensub*srate,1:size(obt,2)),synstf(1:lensub*srate,1:size(obt,2)),...
%     (1:lensub*srate)/srate,rct,0.05,mm(1:size(obt,2),:),repmat('P/Z',...
%         [size(obt,2),1]))
% end
