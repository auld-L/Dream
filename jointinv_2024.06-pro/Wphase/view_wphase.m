...
datafolder = 'D:\matlab2021\codes\jtinew2022.09\2023-02-06-mww75-turkeyLH';
%deldex=[6,8,16,17,24,25];
%deldex=[8,16,17,18,24,25,28,30,31,32,39,45,46,50];
%deldex=[8,15,17,24,25,27,28,31,34,37,45];
deldex=[];
band=[0.0017,0.0067];
taper=1:1:leng_before*srate;
taper=1./(leng_before*srate-taper+1);
...
for i = 1:nsta
	obs0 = obs(max(t(i)-leng_before*srate,1):t(i)+round(15*da(i,1)/111.19492664455875*srate),i);
    obs0(1:leng_before*srate)=obs0(1:leng_before*srate).*taper'.^2;
    obs0=cumsum(obs0)/srate;
    x=1:1:Lpoly;
    x2=1:1:round(15*da(i,1)/111.19492664455875*srate);
    y=obs0(leng_before*srate+1:leng_before*srate+Lpoly);
    p=polyfit(x,y,1);
    q=polyval(p,x2);
    obs0(leng_before*srate+1:leng_before*srate+round(15*da(i,1)/111.19492664455875*srate))=obs0(leng_before*srate+1:leng_before*srate+round(15*da(i,1)/111.19492664455875*srate))-q';
...
save ob_tele_Wphase75LH.mat ob loca da mm srate leng leng_before
