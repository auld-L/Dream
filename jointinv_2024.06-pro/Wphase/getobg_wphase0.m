function [ob,g,loca,mm,srate,leng,ga] = getobg_wphase0(weight,ifrecal,obfile,epi,depth,fault,grid,gridsize,source,gpath,miu,band,isdis)
...
if weight
    if ifrecal
        ...
        for k=1:nsub
            t(k,:)=round(get_time_new(distdaa(k,:)/111.19492664455875,dep(k),'P')*srate);
        end
            %t=reshape(t,nsub,nsta);
        t0=round(get_time_new(da(:,1)/111.19492664455875,depth,'P')*srate);
        for i=1:6
            M=zeros(6,1);M(i)=1;
            ...
            for j=1:size(gtm,3)
                for k=1:nsub
                    ...
                    gt(ttt+1:end,i,k,j)=0;
%                     tap=(sind(90:90/(50*srate):1)).^2;
%                     gt(ttt-50*srate:end,i,k,j)=gt(ttt-50*srate:end,i,k,j).*tap;
                end
            end
        end
        for i=1:nsub
...
%         g = permute(gt,[1,2,4,3]);
    ...
    m=1;
            for j=1:size(gtm,3)
                for k=1:nsub
                    ttt=round((leng_before+15*daa(m,1)/111.19492664455875)*srate);
                    g(ttt+1:end,:,k,j)=0;
                    m=m+1;
                end
