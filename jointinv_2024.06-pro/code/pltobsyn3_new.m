function [misfitsmi,corsmi] = pltobsyn3_new(ob,syn,t,rc,rat,mm)
%==========================================================================
% ob: [leng,3,nsta]
% syn: [leng,3,nsta]
...
fit=gfit1(ob,syn);
szo=size(fit);
msft=sum((ob-syn).^2)./sum(ob.^2);
misfitsmi(:,:)=reshape(msft,szo(2),szo(3));
corsmi(:,:)=reshape(fit,szo(2),szo(3));
figure('color','w')
...
%     end
    ...
    for j=1:size(mm0,1)
        ...
        text(nt,yzero((j-1)*3+1)-0.2,num2str(abs(fit(1,1,rc(1)*(i-1)+j)),'%.2f'),'fontsize',fsize,'HorizontalAlignment','left','VerticalAlignment','top');
        text(nt,yzero((j-1)*3+1)-2.2,num2str(abs(fit(1,2,rc(1)*(i-1)+j)),'%.2f'),'fontsize',fsize,'HorizontalAlignment','left','VerticalAlignment','top');
        text(nt,yzero((j-1)*3+1)-4.2,num2str(abs(fit(1,3,rc(1)*(i-1)+j)),'%.2f'),'fontsize',fsize,'HorizontalAlignment','left','VerticalAlignment','top');
        text(nt+t(end)*0.8,yzero((j-1)*3+1)-0.2,num2str(abs(msft(1,1,rc(1)*(i-1)+j)),'%.2f'),'fontsize',fsize,'HorizontalAlignment','left','VerticalAlignment','top','color','r');
        text(nt+t(end)*0.8,yzero((j-1)*3+1)-2.2,num2str(abs(msft(1,2,rc(1)*(i-1)+j)),'%.2f'),'fontsize',fsize,'HorizontalAlignment','left','VerticalAlignment','top','color','r');
        text(nt+t(end)*0.8,yzero((j-1)*3+1)-4.2,num2str(abs(msft(1,3,rc(1)*(i-1)+j)),'%.2f'),'fontsize',fsize,'HorizontalAlignment','left','VerticalAlignment','top','color','r');
    end
    axis tight
