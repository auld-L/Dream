function [ob,g,loca,mm,srate,leng,twstop] = getobg_wphase1d(weight,ifrecal,obfile,epi,depth,fault,faultall,gridsize,source,gpath,miu,band,isdis)
%(weight,ifrecal,obfile,depth,grid,gpath,miu,band,isdis)
% 鑾峰緱杩滈渿璁板綍鐨勮娴嬫尝褰紝璁＄畻鏍兼灄鍑芥暟锛屽苟杩涜婊ゆ尝
if weight
    if ifrecal
        load(obfile);
...
%             t=diag(t2);
            for j=1:size(gtm,3)
                ...
                gt(:,i,j)=gtm(round(t0(j)-leng_before*srate):round(t0(j)-leng_before*srate)+size(ob,1)-1,1,j);
                ttt=round((leng_before+15*daa(j,1)/111.19492664455875)*srate);
                %gt(ttt+1:end,i,j)=0;   %change  try
...
%                 gt(ttt-50*srate:end,i,j)=gt(ttt-50*srate:end,i,j).*tap;
    ...
    for j=1:size(ob,2)
        ...
        ob(ttt+1:end,j)=0;
            ...
            save obg_wphase.mat ob g loca mm srate leng twstop
    else
        load('obg_wphase.mat');
        disp('Wave and Green''s function of wphase record have been loaded!');
    end
else
    ob = [];g = [];loca = [];mm = [];srate = 0;leng = 0;twstop=[];
end
end
