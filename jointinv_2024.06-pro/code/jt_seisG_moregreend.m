function [G,sGDT,hdata,meangreen]=jt_seisG_moregreend(fault,trup_con0,delt, ...
    lensub,lambda1,lambda2,lambda3,obs,green,inter,srate,rake,faultall,biaoall,difsrate)
%==========================================================================
...
%G=conG_wchds(green,delt,fault,lensub,trup_con,inter,rake,difsrate);
G=conG_wchds2(green,delt,fault,lensub,trup_con,inter,rake,difsrate);
%G=conG_wch(green,delt,fault,grid,ndot,trup_con,inter,rake)
load('gridpai.mat');
% constrain in space:
if lambda1~=0
    D1=sm_space_inter(gridpai(1,:),lensub);
    aa=size(D1);
if size(gridpai,1)>1
for jj=2:size(gridpai,1)
    %dex1=find(biaoall(:,1)==1);
    D2=sm_space_inter(gridpai(jj,:),lensub);
    bb=size(D2);
    %D(xtrup_con(end:-1:1),:)=[];
    D=[[D1,sparse(aa(1),bb(2))];[sparse(bb(1),aa(2)),D2]];
    D1=D;
    aa=size(D1);
end
else
...
% constrain in time:
if lambda2~=0
    [T]=sm_time_inter([size(faultall,1),1],lensub);
    T(:,xtrup_con(end:-1:1))=[];
    %T(xtrup_con(end:-1:1),:)=[];
