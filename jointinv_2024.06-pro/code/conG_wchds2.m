function G=conG_wchds2(green,delt,fault,ndot,trup_con,inter,rake,difsrate)
...
%             sG2=subG2;sG2(:,zt)=[];%subG2(:,zt)=[];
            ...
            for ijk=1:numel(delt0)                
                ...
                tshift=floor(((j-1)*inter+delt0(ijk))*difsrate(i))+1;
                if difsrate(i)<1
            
                tshift1=round(((j-1)*inter+delt0(ijk)));
                yu=mod(tshift1,1/difsrate(i));
                sG11up=resample(sG11(:,ijk),1/difsrate(i),1);
                sG22up=resample(sG22(:,ijk),1/difsrate(i),1);
                n=leng-tshift;
                G1tmp=sG11up(1/difsrate(i)-yu:1/difsrate(i):n/difsrate(i)-yu);
                G2tmp=sG22up(1/difsrate(i)-yu:1/difsrate(i):n/difsrate(i)-yu);
                G((i-1)*leng+tshift+1:i*leng,kk+ijk-1)=G1tmp;
                G((i-1)*leng+tshift+1:i*leng,kk+ijk-1+length(onet))=G2tmp;    
                else
    ...
    for i=1:nsta
        ...
        for j=1:ndot
            ...
            for ijk=1:numel(delt0)
                ...
                tshift1=round(((j-1)*inter+delt0(ijk)));
                yu=mod(tshift1,1/difsrate(i));
                sG11up=resample(subG(:,ijk),1/difsrate(i),1);
            ...
            G((i-1)*leng+tshift+1:i*leng,kk+ijk-1)=subG(1:end-tshift,ijk);
                end
            end
            kk=kk+ss(2);
