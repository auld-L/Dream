function [slip,stf,slipx,slipy,slipe,slipn,zslip,stf_subout]=get_result2d(X,lensub,faultall,gridsize,fault,miu,srate)
%personal change in
fault0=fault;
fault=[];
fault(:,1:2)=faultall(:,4:5);
fault(:,3)=fault0(3);
if nargin<6
    miu=3e10;
end
...
nsub=size(faultall,1);
if numel(X)>nsub*lensub
    X1=sqrt(X(:,1).^2+X(:,2).^2);

    stf_sub=reshape(X1,[lensub,nsub]);
    %stf_subout=stf_sub*diag(miu)/3e10;%/prod(gridsize)/1e6;%%%add
    stf_subout=stf_sub;%add
    
    stf=sum(stf_sub/3e10*diag(miu(:)),2)*srate;
    %stf=sum(sum(reshape(X1,[lensub,sizemat]),2),3);
    
    Xslip=X/3e16/prod(gridsize);
    slip1(:)=sum(reshape(Xslip(:,1),[lensub,size(faultall,1)]));
    slip1=slip1';
    slip2(:)=sum(reshape(Xslip(:,2),[lensub,size(faultall,1)]));
    slip2=slip2';
...
stf_sub=reshape(X,[lensub,nsub]);%%%%add
stf=sum(stf_sub/3e10*diag(miu(:)),2)*srate;
stf_subout=stf_sub;%%%add
Xslip=X/3e16/prod(gridsize);
slip=sum(reshape(Xslip,[lensub,nsub]));
slip=slip';
% slip=sum(reshape(X./nsub./1e6./3e10,[lensub,nsub]));
% slip=slipnf(sizemat,slip);

%stf=sum(sum(reshape(X,[lensub,sizemat]),2),3)*srate;
slipx=slip.*cosd(fault(:,3));
slipy=slip.*sind(fault(:,3));
% slipx=slipnf(sizemat,slipx);
% slipy=slipnf(sizemat,slipy);
slipe=[];
slipn=[];
