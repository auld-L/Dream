function [stf,rsq,stfall]=pldfp(obs,g,ll,itern,aerfa)
% =========================================================================
...
for i=1:so(2)
    ...
    to=1+logspace(log10(aerfa),-1,fftlen);
    M1(:,i)=to(:).*D(:,i).*Mpg(:,i);%M1=1-to.*D.*G'*G
    M2(:,i)=to(:).*D(:,i).*pgg(:,i).*pobs(:,i);%M2=to.*D.*G'*obs
end
M1=1-M1;
